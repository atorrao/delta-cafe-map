...
const UI = {
  ...
  renderProfile: function() {
    ...
    if (photoData) {
      html += '<img src="' + photoData + '" class="ph-photo" alt="Foto de perfil" id="ph-photo-img">';
    } else {
      html += '<div class="ph-photo ph-photo-avatar" id="ph-photo-img">' + Gamification.getAvatarSVG(pts, u.selectedAvatar) + '</div>';
    }
    html += '<label class="ph-photo-upload-btn" for="ph-photo-file" title="Alterar foto">';
    ...
    html += '</label>';
    html += '<input type="file" id="ph-photo-file" accept="image/*" style="display:none;">';
    html += '</div>';
    ...
    [['Primeiro local adicionado','+20 pts bónus'],['Adicionar um local','+5 pts'],['Adicionar fotografia','+3 pts']].forEach(function(e){
